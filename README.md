# 👋 Merhaba, ben Emre Can Karacaağaçlı

**Teknik Analist | Bankacılık Teknolojileri | API & Sistem Analizi | Web Arayüzleri**

Bankacılık teknolojileri alanında çalışan bir teknik analist olarak; API analizleri, sistem entegrasyonları, backend servis akışları ve teknik dokümantasyon süreçleri üzerine çalışıyorum.

İş birimlerinden gelen ihtiyaçları teknik gereksinimlere dönüştürerek, yazılım ekipleriyle iş ekipleri arasında köprü kurmayı hedefliyorum.

---

## 🚀 Odaklandığım Alanlar

- API ve sistem analizi
- Backend servis ve entegrasyon akışları
- İş gereksinimlerinin teknik çözüme dönüştürülmesi
- Frontend - backend iletişim yapılarının analizi
- Teknik dokümantasyon ve analiz standartları
- Bankacılık sistemleri ve operasyonel süreçler
- Kullanıcı arayüzü odaklı web projeleri

---

## 🛠️ Kullandığım Araçlar & Teknolojiler

![Azure DevOps](https://img.shields.io/badge/Azure%20DevOps-0078D7?style=flat&logo=azuredevops&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat&logo=postman&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-025E8C?style=flat&logo=postgresql&logoColor=white)
![REST API](https://img.shields.io/badge/REST%20API-000000?style=flat&logo=fastapi&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![HTML](https://img.shields.io/badge/HTML-E34F26?style=flat&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS-1572B6?style=flat&logo=css3&logoColor=white)

---

## 💼 Profesyonel Bakış Açım

Analiz tarafında yalnızca gereksinimi yazıya dökmeye değil; sistemin nasıl çalıştığını anlamaya, servisler arası akışı netleştirmeye ve sürdürülebilir teknik çözümler üretmeye odaklanıyorum.

GitHub tarafında ise teknik analiz bakış açımı; kullanıcı arayüzleri, web projeleri, deneysel tasarımlar ve küçük ölçekli ürün fikirleriyle destekliyorum.

---

## 📌 Öne Çıkan Proje Alanlarım

- Lojistik ve operasyon yönetimi arayüzleri
- Web tabanlı kullanıcı deneyimi tasarımları
- Mobil uyumlu landing page çalışmaları
- Dashboard ve kontrol paneli yapıları
- Deneysel UI / UX projeleri
- API destekli iş akışı fikirleri

---

## 📊 GitHub İstatistikleri

![Emre's GitHub stats](https://github-readme-stats.vercel.app/api?username=Scourge14&show_icons=true&theme=github_dark&hide_border=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Scourge14&layout=compact&theme=github_dark&hide_border=true)

---

## 🌐 Bağlantılar

[![GitHub](https://img.shields.io/badge/GitHub-Scourge14-181717?style=flat&logo=github)](https://github.com/Scourge14)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Emre%20Can%20Karacaağaçlı-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/emre-can-karacaa%C4%9Fa%C3%A7l%C4%B1-650945264/)

---

> Analiz, sistem akışı ve kullanıcı deneyimi arasında güçlü bir bağ kurmaya çalışıyorum.
